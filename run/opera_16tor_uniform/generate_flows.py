#!/usr/bin/env python3
"""Generate a balanced one-flow-per-host workload for the 16-ToR Opera topology."""
...
DEFAULT_OFFSETS = (1, 5, 9, 13)
DEFAULT_FLOW_SIZE_BYTES = 32 * 1024 * 1024
DEFAULT_SUPERSLICE_NS = 14_500
...
def build_flows(
    ...
    if len(offsets) != HOSTS_PER_TOR or len(set(offsets)) != HOSTS_PER_TOR:
        raise ValueError("exactly four distinct ToR offsets are required")
    if 0 in offsets:
    ...
    for source_tor in range(TORS):
        for lane in range(HOSTS_PER_TOR):
            ...
            if start_mode == "cycle_spread":
                start_slice = (source_tor + HOSTS_PER_TOR * lane) % TORS
...
def validate_flows(flows, start_mode):
    ...
    if set(source_tors.values()) != {HOSTS_PER_TOR}:
        raise RuntimeError("each ToR must source exactly four flows")
    if set(destination_tors.values()) != {HOSTS_PER_TOR}:
        raise RuntimeError("each ToR must receive exactly four flows")
    ...
    if start_mode == "cycle_spread":
        ...
        if starts != Counter({slice_index: HOSTS_PER_TOR for slice_index in range(TORS)}):
            raise RuntimeError("cycle-spread mode must start four flows per superslice")
...
def main():
    ...
    print(
        f"Each flow: {flow_size_bytes} bytes ({flow_size_bytes / (1024 * 1024):.3f} MiB)"
    )
    print("Every host sources one flow and receives one flow")
    print("Every ToR sources four flows and receives four flows")
    print(f"Destination ToR offsets by host lane: {tuple(args.offsets)}")
    if args.start_mode == "cycle_spread":
        print(
            "Start distribution: "
            + ", ".join(f"SS{index}={starts[index]}" for index in range(TORS))
        )
    else:
