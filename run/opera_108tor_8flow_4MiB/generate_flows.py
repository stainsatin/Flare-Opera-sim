#!/usr/bin/env python3
"""Generate the balanced 8-flow-per-host Opera priority workload."""
...
DEFAULT_ACTIVE_WINDOW_NS = 44_500
...
def build_flows(
    ...
    for source_tor in range(TORS):
        for lane in range(HOSTS_PER_TOR):
            ...
            for offset_rank, offset in enumerate(offsets):
                ...
                # Spread each receiver's starts across the active window. The
                # destination term also gives every flow a distinct timestamp.
    ...
    validate_flows(flows, offsets, start_superslices, active_window_ns,
                   superslice_ns)
    return flows


def validate_flows(flows, offsets, start_superslices, active_window_ns,
                   superslice_ns):
    ...
    if any(
        flow["source"] % HOSTS_PER_TOR
        != flow["destination"] % HOSTS_PER_TOR
        for flow in flows
    ):
    ...
    if set(tor_pairs.values()) != {HOSTS_PER_TOR}:
        raise RuntimeError("every selected ToR pair must carry six flows")
