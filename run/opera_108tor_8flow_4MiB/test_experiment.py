...
class OperaEightFlowExperimentTest(unittest.TestCase):
    ...
    def test_offsets_and_tor_matrix_are_symmetric_and_balanced(self):
        ...
        self.assertEqual(offsets, [7, 20, 34, 47, 61, 74, 88, 101])
        ...
        self.assertEqual(len(tor_pairs), 108 * 8)
        self.assertEqual(set(tor_pairs.values()), {6})
        self.assertEqual(
            Counter(flow["source_tor"] for flow in flows),
            Counter({tor: 48 for tor in range(108)}),
        )
        self.assertEqual(
    ...
    def test_starts_are_balanced_unique_and_inside_active_windows(self):
        ...
        self.assertEqual(len({flow["start_ns"] for flow in flows}), 5_184)
        ...
        self.assertLess(max(flow["start_ns"] for flow in flows), 2 * 54_500)
    ...
    def test_trace_has_four_columns_and_all_flows(self):
        ...
        self.assertEqual(len(rows), 5_184)
    ...
    def test_invalid_release_configuration_is_rejected(self):
        ...
        with self.assertRaises(ValueError):
            FLOWS.build_flows(base_start_ns=45_000)

    def test_run_script_has_fifo_priority_and_requested_defaults(self):
        ...
        self.assertIn("dynexp_55us_symm.txt", script)
        self.assertTrue((ROOT / "topologies/dynexp_55us_symm.txt").is_file())
