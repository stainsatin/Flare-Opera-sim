...
SHAPING_QUEUE=""
DATA_QUEUE=256
OUTPUT_DIR="${SCRIPT_DIR}/results_isolated"
BUILD=yes
...
usage() {
    cat <<'EOF'
Usage: bash run/multibottleneck_credit_order/run_isolated.sh [options]

Options:
  --simtime SECONDS     Simulation duration (default: 0.5)
  --utiltime MS         Utilization sampling interval (default: 0.5)
  --flow-size BYTES     Bytes per finite flow (default: 10000000)
  --rounds COUNT        Five-flow arrival waves (default: 20)
  --interval-ns NS      Time between waves (default: 20000000)
  --case NAME           Run short_only or long_only
...
for case_name in "${CASES[@]}"; do
    ...
    python3 "${SCRIPT_DIR}/generate_flows.py" \
        ...
        --order-gap-ns 0 \
        --output "${flowfile}"
    echo "Running isolated case=${case_name} for ${SIMTIME}s..."
    "${SIMULATOR}" \
        -flare \
        -strat single \
        -simtime "${SIMTIME}" \
...
python3 "${SCRIPT_DIR}/analyze_isolated.py" "${OUTPUT_DIR}" \
