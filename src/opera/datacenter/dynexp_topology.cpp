...
DynExpTopology::DynExpTopology(mem_b queuesize, Logfile* lg, EventList* ev,queue_type q, 
        string topfile, map<string,uint64_t> params){
...
DynExpTopology::DynExpTopology(mem_b queuesize, Logfile* lg, EventList* ev,queue_type q, 
        string topfile) : DynExpTopology(queuesize, lg, ev, q, topfile, (map<string,uint64_t>){}) {}   
...
void DynExpTopology::set_params() {
    ...
    cout << "set_params " << _no_of_nodes << " " << _ndl << endl;
}
...
Queue* DynExpTopology::alloc_src_queue(DynExpTopology* top, QueueLogger* queueLogger, int node) {
  if(qt==CREDIT) {
    ...
    if (_params.count("rx_hop_weight_medium"))
        medium_weight = _params["rx_hop_weight_medium"];
...
Queue* DynExpTopology::alloc_queue(QueueLogger* queueLogger, uint64_t speed, mem_b queuesize, int tor, int port) {
    if (qt==COMPOSITE)
        ...
        return new INTQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger, tor, port, this);
    else if (qt==CREDIT)
        return new CreditQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger, tor, port, this, 
        _params["cq_size"], _params["sh_thresh"], _params["ae_thresh"], _params["te_thresh"]);
    else if (qt==BOLT)
        return new BoltQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger, tor, port, this);
    else if (qt==HBH)
        return new HbHQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger, tor, port, this,
        _params["hbh_D"], _params["hbh_P"]);
    assert(0);
}
